...
func NewRedisScheduler(addr, password string, db, maxConn, maxIdle int, forbiddenDuplicateUrl bool) *RedisScheduler {
	rs := &RedisScheduler{
		redisAddr:             addr,
		redisPass:             password,
		redisDb:               db,
...
func (this *RedisScheduler) newConn() (redis.Conn, error) {
	...
	if _, err := c.Do("AUTH", this.redisPass); err != nil {
		c.Close()
		return nil, err
	}
	if _, err := c.Do("SELECT", this.redisDb); err != nil {
		c.Close()
		return nil, err
	}
	return c, nil
